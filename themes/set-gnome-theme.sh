...
# accent-color's enum (GNOME 47+) doesn't line up with the Yaru palette names, so themes
# set OMAKUB_GNOME_ACCENT separately; the fallback covers themes/GNOME versions that don't.
gsettings set org.gnome.desktop.interface accent-color "${OMAKUB_GNOME_ACCENT:-$OMAKUB_THEME_COLOR}" 2>/dev/null || true
